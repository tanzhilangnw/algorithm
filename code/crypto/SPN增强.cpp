...
using namespace std;
unsigned long long S_BOX[2][16] = { { 14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7 }, {14, 3, 4, 8, 1, 12, 10, 15, 7, 13, 9, 6, 11, 2, 0, 5} };
...
unsigned long long SPN(unsigned long long k[5], unsigned long long plaintext, int mode) {
	for (int i = 0; i < 3; i++) {
		if (mode == 1) {
			...
			plaintext ^= k[i];
		plaintext = S_BOX[mode][plaintext & 0xf] |
			((S_BOX[mode][(plaintext & 0xf0) >> 4]) << 4) |
			((S_BOX[mode][(plaintext & 0xf00) >> 8]) << 8) |
			((S_BOX[mode][(plaintext & 0xf000) >> 12]) << 12) |
			((S_BOX[mode][(plaintext & 0xf0000) >> 16]) << 16) |
			((S_BOX[mode][(plaintext & 0xf00000) >> 20]) << 20) |
			((S_BOX[mode][(plaintext & 0xf000000) >> 24]) << 24) |
			((S_BOX[mode][(plaintext & 0xf0000000) >> 28]) << 28) |
			((S_BOX[mode][(plaintext & 0xf00000000) >> 32]) << 32) |
			((S_BOX[mode][(plaintext & 0xf000000000) >> 36]) << 36) |
			((S_BOX[mode][(plaintext & 0xf0000000000) >> 40]) << 40) |
			((S_BOX[mode][(plaintext & 0xf00000000000) >> 44]) << 44) |
			((S_BOX[mode][(plaintext & 0xf000000000000) >> 48]) << 48) |
			((S_BOX[mode][(plaintext & 0xf0000000000000) >> 52]) << 52) |
			((S_BOX[mode][(plaintext & 0xf00000000000000) >> 56]) << 56) |
			((S_BOX[mode][(plaintext & 0xf000000000000000) >> 60]) << 60);
