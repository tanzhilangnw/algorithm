...
bool isPrime(mpz_t paraN){
        ...
        mpz_mod_ui(b, t, 2);//t = t mod 2
        while(mpz_cmp_ui(b, 0) == 0){           //  求解t
                mpz_div_ui(t, t, 2);
                mpz_mod_ui(b, t, 2);
                count++;
        }
        mpz_sub_ui(t,n,1);//t = t-1
        if(count == 0){
                return false;           // 偶数直接排除
        }
        ...
        for(int i = 0; i < 300; i++){
                //mpz_init_set_ui(a, 4);
                mpz_urandomm(a, state, n);              // 随机选择a 生成0 ~ n-1的整数
                if(mpz_cmp_ui(a, 0) != 0 && mpz_cmp_ui(a, 1) != 0){
                        mpz_powm(temp, a, t, n);//temp = a^t mod n
                        ...
                        for(int j = 0; j < count; j++){
                                ...
                                mpz_powm_ui(temp, temp, 2, n);//temp = temp ^ 2 mod n
                        }
                }
        }
        //gmp_printf("%Zd\n",n);
...
bool gcdIsOne(mpz_t paraA, mpz_t paraB){
        ...
        mpz_init(a);
        mpz_init(b);
        mpz_init_set(a, paraA);//a=paraA
        mpz_init_set(b, paraB);//b=paraB
        mpz_mod(temp, a, b);//temp = a mod b
        //辗转相除
...
void calD(mpz_t e, mpz_t p, mpz_t q){
        ...
        // 求解d
        mpz_init_set(temp,e);
        mpz_init_set(temp2,phi);
        mpz_powm(temp1,e,-1,phi);
        /*mpz_mod(temp, e, phi);
