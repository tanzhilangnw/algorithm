...
int main(){
	...
	for(int i = 0; i < n; ++i){
		...
		for(int group = 1; group <= 8000; ++group){
			...
			//
            x[5] = (plaintext[group]>>11)& 0x1;  
            x[7] = (plaintext[group]>>9)& 0x1;
            x[8] = (plaintext[group]>>8)& 0x1;

			//y_init = ciphertext[group];
			//for(int pos = 1, k = 12;pos<=4&&(pos==2||pos==4); ++pos, k -= 4){
			//	y[pos] = (y_init >> k) & 0xf;
			//}
            y[2] =(ciphertext[group]>> 8) & 0xf;
            y[4] = ciphertext[group]&0xf;
			
			for(int L1 = 0; L1 < 16; ++L1){
				for(int L2 = 0; L2 < 16;++L2){
					u2 = inverse_sBox[L1 ^ y[2]];
					u4 = inverse_sBox[L2 ^ y[4]];
					//int z = (x[5] ^ x[7] ^ x[8] ^ (u2 >> 2) ^ u2 ^ (u4 >> 2) ^ u4) & 0x1;
					if(!((x[5] ^ x[7] ^ x[8] ^ (u2 >> 2) ^ u2 ^ (u4 >> 2) ^ u4) & 0x1)) cnt24[L1][L2]++;
				}
			}
		}
		...
		for(int round24 = 0; round24 < 64; ++round24){
			...
			for(int group = 1; group <= 8000; ++group){
				...
				//	x[pos] = (x_init >> (16 - pos)) & 0x1;
				//}
                x[1] = (plaintext[group]>>15)& 0x1;  
                x[2] = (plaintext[group]>>14)& 0x1;  
                x[4] = (plaintext[group]>>12)& 0x1;  
                x[9] = (plaintext[group]>>7)& 0x1;  
                x[10] = (plaintext[group]>>6)& 0x1;  
                x[12] = (plaintext[group]>>4)& 0x1;  

              
				//y_init = ciphertext[group];
				for(int pos = 1, k = 12; pos <= 4; ++pos, k -= 4){
					y[pos] = (ciphertext[group] >> k) & 0xf;
				}
				//开始计算count 
				for(int L1 = 0; L1 < 16; ++L1){
					for(int L2 = 0; L2 < 16; ++L2){
			...
			for(int round13 = 0; round13 < 2; ++round13){
				...
				for(fore_key = 0; fore_key <=65535;++fore_key){
